...
exports.sendSupportReply = onCall(async (request) => {
  ...
  // Only allow the app owner (admin)
  if (!ADMIN_UID || request.auth.uid !== ADMIN_UID) {
...
exports.getLeaderboard = onCall(async (request) => {
  ...
  if (period === "weekly" || period === "daily") {
    // For time-filtered leaderboards, fetch by recency then sort by likes in memory.
    // This avoids a complex composite index (range on createdAt + orderBy likeCount).
    ...
    query = query
      ...
      .limit(resultLimit * 3); // fetch extra to find top-liked
    needsMemorySort = true;
  } else {
    // all_time: single composite index (reported, likeCount, createdAt)
    query = query
  ...
  if (needsMemorySort) {
    quotes.sort((a, b) => b.likeCount - a.likeCount || b.createdAt - a.createdAt);
    quotes.splice(resultLimit); // trim to requested limit
  }
